...
import { RefreshCcw, ChevronDown, ChevronUp, Loader2, AlertCircle, Sparkles } from 'lucide-react';
...
export function LibrarySyncNotice({ projectReferences, onReferencesReplaced }: LibrarySyncNoticeProps) {
    ...
    const check = useCallback(async () => {
        try {
            const response = await fetch('/api/data/character-library/check-updates', {
                ...
                body: JSON.stringify({ references: projectReferences }),
            });
    ...
    useEffect(() => {
        if (!projectReferences.some((ref) => ref.sourceCharacterLibraryItemId)) {
            setDiffs([]);
            setHasChecked(true);
            return;
        }
        void check();
    }, [check, projectReferences]);
    ...
    if (!hasChecked || diffs.length === 0) return null;
