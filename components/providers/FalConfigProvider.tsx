...
export function FalConfigProvider({ children }: { children: React.ReactNode }) {
    useEffect(() => {
        ...
        if (localApiKey) {
            // 如果用戶設定了自己的 API key，直接使用
            fal.config({
                credentials: localApiKey,
            });
            console.log('✅ Fal Client: 使用本地 API key');
        } else {
        ...
        // 監聽 localStorage 變化（當用戶在設定中更新 API key）
        const handleStorageChange = (e: StorageEvent) => {
            if (e.key === 'fal_api_key') {
                if (e.newValue) {
                    ...
                    console.log('🔄 Fal Client: 切換到本地 API key');
                } else {
                    fal.config({
                        proxyUrl: '/api/fal/proxy',
                    });
