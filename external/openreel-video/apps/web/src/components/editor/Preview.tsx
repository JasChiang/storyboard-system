...
export const Preview: React.FC = () => {
  ...
  const hasLoggedTransitionFallbackRef = useRef<boolean>(false);
  ...
  const canUseNativeVideoPlayback = useCallback(
    (
      ...
      if (tracks.some((track) => track.transitions?.length > 0)) {
        if (!hasLoggedTransitionFallbackRef.current) {
          console.info(
            "[Preview] Transitions detected. Falling back to composited playback.",
          );
          hasLoggedTransitionFallbackRef.current = true;
        }
