...
import type { Transition, Clip, Track, Project } from "@openreel/core";
...
export class TransitionBridge {
  ...
  /**
   * Replace transitions for a track (used when loading an external project)
   */
  ...
  /**
   * Hydrate transitions from a project file
   */
  loadTransitionsFromProject(project: Project): void {
    this.clearAllTransitions();
    for (const track of project.timeline.tracks) {
      if (track.transitions && track.transitions.length > 0) {
        this.setTransitionsForTrack(track.id, track.transitions);
      }
    }
  }
